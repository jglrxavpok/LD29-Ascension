...
public class LWJGLHandler
{
	...
	{
		System.out.println("OS found : "+System.getProperty("os.name")+ " "+System.getProperty("os.version"));
		...
		if(getNameOfOS().equalsIgnoreCase("windows"))
		{
			...
			{
				OutputStream out = IO.copy(getFFJar("/windows/jinput-dx8_64.dll", LWJGLHandler.class), folder.getPath()+"/jinput-dx8_64.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/jinput-dx8.dll", LWJGLHandler.class), folder.getPath()+"/jinput-dx8.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/jinput-raw_64.dll", LWJGLHandler.class), folder.getPath()+"/jinput-raw_64.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/jinput-raw.dll", LWJGLHandler.class), folder.getPath()+"/jinput-raw.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/lwjgl.dll", LWJGLHandler.class), folder.getPath()+"/lwjgl.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/lwjgl64.dll", LWJGLHandler.class), folder.getPath()+"/lwjgl64.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/OpenAL32.dll", LWJGLHandler.class), folder.getPath()+"/OpenAL32.dll");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/windows/OpenAL64.dll", LWJGLHandler.class), folder.getPath()+"/OpenAL64.dll");
				out.flush();
				out.close();
			}
			else
		...
		if(getNameOfOS().equalsIgnoreCase("solaris"))
		{
			...
			{
				OutputStream out = IO.copy(getFFJar("/solaris/liblwjgl.so", LWJGLHandler.class), folder.getPath()+"/liblwjgl.so");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/solaris/liblwjgl64.so", LWJGLHandler.class), folder.getPath()+"/liblwjgl64.so");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/solaris/libopenal.so", LWJGLHandler.class), folder.getPath()+"/libopenal.so");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/solaris/libopenal64.so", LWJGLHandler.class), folder.getPath()+"/libopenal64.so");
				out.flush();
				out.close();
			}
			else
		...
		if(getNameOfOS().equalsIgnoreCase("linux"))
		{
			if(!new File(folder.getPath()+"/liblwjgl.so").exists())
			{
				OutputStream out = IO.copy(getFFJar("/linux/liblwjgl.so", LWJGLHandler.class), folder.getPath()+"/liblwjgl.so");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/linux/liblwjgl64.so", LWJGLHandler.class), folder.getPath()+"/liblwjgl64.so");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/linux/libopenal.so", LWJGLHandler.class), folder.getPath()+"/libopenal.so");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/linux/libopenal64.so", LWJGLHandler.class), folder.getPath()+"/libopenal64.so");
				out.flush();
				out.close();
			}
			else
		...
		if(getNameOfOS().equalsIgnoreCase("macosx"))
		{
			...
			{
				OutputStream out = IO.copy(getFFJar("/macosx/liblwjgl.jnilib", LWJGLHandler.class), folder.getPath()+"/liblwjgl.jnilib");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/macosx/liblwjgl-osx.jnilib", LWJGLHandler.class), folder.getPath()+"/liblwjgl-osx.jnilib");
				out.flush();
				out.close();
				out = IO.copy(getFFJar("/macosx/openal.dylib", LWJGLHandler.class), folder.getPath()+"/openal.dylib");
				out.flush();
				out.close();
			}
			else
		...
		System.out.println("Natives have been install!\nConfiguring LWJGL paths...");
		System.out.println("Paths configured!\nConfiguring JInput's paths...");
